...
export async function Insights() {
  const [{ t }, data] = await Promise.all([getServerT(), getInsights()])
  ...
  return (
    <Panel id={ID}>
      <PanelHeader>
        <PanelTitle>
          <a href={`#${ID}`}>{t("section.insights")}</a>
          <PanelTitleSup>
      ...
      <div className="relative">
        ...
        <dl className="grid grid-cols-2 sm:grid-cols-3">
          <Metric>
            <MetricLabel>{t("label.uniqueVisitors")}</MetricLabel>
            <MetricValue>
          ...
          <Metric>
            <MetricLabel>{t("label.sessions")}</MetricLabel>
            <MetricValue>
          ...
          <Metric>
            <MetricLabel>{t("label.views")}</MetricLabel>
            <MetricValue>
      ...
      {data.series.length > 0 ? (
        ...
        >
          ...
          <ChartTooltip
            rowLabels={{
              total_sessions: t("label.sessions"),
              unique_visitors: t("label.uniqueVisitors"),
            }}
          />
        </LineChart>
      ) : (
        <div className="grid aspect-2/1 w-full place-content-center sm:aspect-3/1">
          <p className="text-muted-foreground">{t("label.noInsights")}</p>
        </div>
      )}
